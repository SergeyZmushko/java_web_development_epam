...
оператор foreach применим к массивам и классам, реализующим интерфейс java.lang.Iterable. Foreach работает напрямую с элементом, в то время как for работает с итерационной переменной.  
1 - «Цикл со счетчиком»;  
2 - «Для каждого».  
...
public static void main(String...args) { }  
можем добавить strictfp для метода main() , который используется для совместимости между процессорами при работе со значениями с плавающей запятой:  
